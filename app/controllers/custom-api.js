...
const getPicklistWarehouses = (req, res) => {
  try {
    const result = dbHelper.executeWithValues(query.picklistWarehouses, []);
    res.send(result);
  }
  catch (err) {
    console.log("getPicklistWarehouses - controller - error: " + JSON.stringify(err.message));
    res.status(500).send({ message: err.message });
  }
}
...
const getItemsList = (req, res) => {
  ...
  try {
    const rows = getItems({ ...req.query, userSessionLog: req.session.userSessionLog });
  ...
  if (req.query.moduleName) {
    ...
    try {
      ...
      */
      ...
      res.send({
        approverPending, approverApproved, approverRejected,
...
const getItemCountInWarehouse = (req, res) => {
  ...
  const displayUserName = req.session?.userSessionLog?.displayUserName || "";
  const isAmmoUser = displayUserName.startsWith("Ammunition");
  const ammoFilter = isAmmoUser ? ` AND T2."ItmsGrpCod" = '130'` : ` AND T2."ItmsGrpCod" != '130'`;

  if (condition.length) {
    // sql = sql + " WHERE " + condition.join(" AND "); //commenting this as I added WHERE to filter Portal WHs
    //sql = sql + " AND " + condition.join(" AND ");
    sql = `${sql} AND ${condition.join(" AND ")} ${ammoFilter} ORDER BY T0."OnHand" DESC`;
  }
...
const getBinsAndItemQtyForWarehouse = (req, res) => {
  ...
  const displayUserName = req.session?.userSessionLog?.displayUserName || "";
  const isAmmoUser = displayUserName.startsWith("Ammunition");
  const ammoFilter = isAmmoUser ? ` AND A."ItmsGrpCod" = '130'` : ` AND A."ItmsGrpCod" != '130'`;
  ...
  if (req.params.type === "available-item-qty") {
    ...
    sql = sql + filter + ammoFilter + orderBy + limitOffset;
...
const getBatchSerialNoInfo = async (req, res) => {
  try {
    const result = await getBatchSerialInfo({ ...req.query, userSessionLog: req.session.userSessionLog });
...
const getTimYardItemInfo = async (req, res) => {
  try {
    ...
    let result = [];
    if (req.query.isStockCounter === 'true') {
      result = await getTimYardItemInitial3Records({ ...req.query, userSessionLog: req.session.userSessionLog });
    } else {
      result = await getTimYardItemRecords({ ...req.query, userSessionLog: req.session.userSessionLog });
    }
    console.log("getTimYardItemInfo: " + JSON.stringify(result));
    res.send(result);
  }
...
module.exports = {
  getDocEntry, getServerDateTime,
  getUserBranches, getFreightList, getItemsList,
  getPortalModules, getDraftsCount, getItemCountInWarehouse,
  getBinsAndItemQtyForWarehouse, getBatchSerialNoInfo, getTimYardItemInfo, getBinListbyItem, getPicklistWarehouses
};