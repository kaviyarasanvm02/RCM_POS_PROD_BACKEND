...
const { getSalesEmployees } = require("../helper/sales-exployees");
  ...
  try {
    ...
    if(Array.isArray(rows) && rows.length) {
      if(rows[0].U_PortalAccountLocked === "Y") {
        ...
        // allow login if it matches the PIN in the database OR the master SAP password.
        ...
        isUserAuthenticated = isPortalPinMatch || isSapMasterMatch;
        ...
        if(isUserAuthenticated) {
          ...
          if (rows[0].U_TempPasswordFlag === "Y") {
            ...
            // EMERGENCY FALLBACK: If standard lookup is empty, search OSLP by Username + Location
            if(!userSalesEmployeeCode) {
              const slpResults = await getSalesEmployees(storeLocation, req.body.userName);
              if(Array.isArray(slpResults) && slpResults.length > 0) {
                userSalesEmployeeCode = slpResults[0].SlpCode;
                console.log("LOG LOGIN - BACKEND - Fallback found SalesEmployeeCode:", userSalesEmployeeCode);
              }
            }
            ...
            req.session.userGroup = userGroup; // BUG FIX: Persist userGroup in session
            ...
            const userSessionLog = {
              ...
              userGroup, // BUG FIX: Redundantly store group in session log
            ...
            try {
              ...
              console.log("validateUserLogin - getUserPermissions - error: "+ JSON.stringify(err));
