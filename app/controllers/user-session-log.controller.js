  ...
  if (req.params.id && req.body) {
    try {
      ...
      if (temp) {
        ...
        // BUG FIX: `counterName` was used as a bare variable (undefined) instead of
        // reading it from `req.body`. This caused the session to store counterName=undefined
        // after a QR code counter scan, so the receipt always printed the stale login-time
        // DB value (e.g. "Bulletin") instead of the newly-selected counter name.
