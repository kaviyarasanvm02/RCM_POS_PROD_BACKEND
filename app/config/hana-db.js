...
const picklistWarehouses = `SELECT "WhsCode" FROM ${dbCreds.CompanyDB}.OWHS WHERE "U_PICKLIST"='Y'`;
...
const getAllBinsForBatch =
  `SELECT A."ItemCode", B."itemName" AS "ItemName", A."WhsCode",B."DistNumber" AS "BatchNumberProperty",A."OnHandQty",C."BinCode" AS "BinCode",
  C."AbsEntry" "BinAbsEntry", D."Location" "LocationCode", E."Location" "LocationName", B."InDate"
FROM ${dbCreds.CompanyDB}.OBBQ A 
  INNER JOIN ${dbCreds.CompanyDB}.OBTN B ON B."AbsEntry"=A."SnBMDAbs" AND A."ItemCode"=B."ItemCode" 
  LEFT JOIN ${dbCreds.CompanyDB}.OBIN C ON C."AbsEntry"=A."BinAbs" AND A."WhsCode"=C."WhsCode"
...
module.exports = {
  ...
  itemsList, itemQuantityInWarehouse, picklistWarehouses,
