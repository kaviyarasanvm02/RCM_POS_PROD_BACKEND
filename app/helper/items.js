...
/**
 * Returns the ammunition filter condition based on the user session
 * @param {Object} userSessionLog 
 * @returns {String}
 */
const getAmmoFilter = (userSessionLog, excludeForNonAmmo = false, alias = 'T0') => {
  if (userSessionLog && userSessionLog.displayUserName && userSessionLog.displayUserName.startsWith("Ammunition")) {
    return ` AND ${alias}."ItmsGrpCod" = '130'`;
  }
  return excludeForNonAmmo ? ` AND ${alias}."ItmsGrpCod" != '130'` : "";
}
...
const getItems = (req) => {
  ...
  const ammoFilter = getAmmoFilter(req.userSessionLog, true, 'T0');
  ...
  let sql = `SELECT T0."ItemCode", T0."ItemName", T0."FrgnName", T0."InvntryUom",
             ...
             WHERE T0."frozenFor" = 'N' ${ammoFilter}`
  //T0."ExitPrice" "Price", 
  ...
  if (req.itemType) {
    ...
    if (req.itemType === itemTypes.NORMAL) {
      sql = `SELECT T0."ItemCode"
              FROM ${query.dbCreds.CompanyDB}.OITM T0
            WHERE T0."ManBtchNum" ='N' AND T0."ManSerNum" ='N' AND T0."frozenFor" = 'N' ${ammoFilter}
              ${itemFilter}`;
      ...
      sql = `SELECT T0."ItemCode"
              FROM ${query.dbCreds.CompanyDB}.OITM T0
            WHERE T0."InvntItem" ='N' AND T0."frozenFor" = 'N' ${ammoFilter}
              ${itemFilter}`;
      //AND T0."ItemCode" IN (${itemCodes})
    }
  }
...
//Here we get the batch/serial info based on the `filters` passed

const getBatchSerialInfo = async (filter) => {
  let sql, values = [], where = "";
  console.log("filter.itemAndWHCodes: " + filter.itemAndWHCodes);

  const ammoFilter = getAmmoFilter(filter.userSessionLog);

  if (filter.type === requestTypes.BATCH_SERIAL_IN_A_BIN) {
    try {
      const result =
        await getBatchSerialRecords(filter.itemType, filter.itemCode, filter.warehouseCode, filter.binCode, filter.userSessionLog);
    ...
    try {
      ...
      batchList = dbHelper.executeWithValues(query.batchForItemAndWH + where + ammoFilter, values);
      serialList = dbHelper.executeWithValues(query.serialForItemAndWH + where + ammoFilter, values);
 ...
 * @returns 
 */
const getBatchSerialRecords = (itemType, itemCode, warehouseCode, binCode, userSessionLog = null) => {
  let condition = [], result, sql;
  const ammoFilter = getAmmoFilter(userSessionLog);
...
const getTimYardItemRecords = (query) => {
  ...
  const ammoFilter = getAmmoFilter(query.userSessionLog);
  if (ammoFilter) {
    condition.push(`T0."ItemCode" IN (SELECT "ItemCode" FROM OITM WHERE "ItmsGrpCod" = '130')`);
  }
  ...
  try {
    ...
    // ✅ Filter out zero-quantity batches at the API level
    if (Array.isArray(result) && result.length > 0) {
      result = result.filter(item => (parseFloat(item.U_AvlQty) > 0 || parseFloat(item.U_AvlPcs) > 0));
    }
...
const getTimYardItemInitial3Records = (query) => {
  ...
  try {
    ...
    if (Array.isArray(enriched) && enriched.length > 0) {
      filtered = enriched.filter(item => (parseFloat(item.U_AvlQty) > 0 || parseFloat(item.U_AvlPcs) > 0));
    }
...
module.exports = { getItems, getBatchSerialInfo, getBatchSerialRecords, setBatchSerialReservedCust, getTimYardItemRecords, getTimYardItemInitial1Records, getTimYardItemInitial3Records, getAmmoFilter }
